...
def render_library_js(entries: list[dict]) -> str:
    ...
    aids = [f"t_album_{i:02d}" for i in range(1, len(tidal) + 1)]
    tidal_playlist_seeds = [
    ...
    return LIBRARY_JS_TEMPLATE % (
        ...
        json.dumps(tidal_playlist_seeds, ensure_ascii=False),
...
def sync_covers(entries: list[dict]) -> tuple[int, int, int]:
...
def main() -> int:
    ...
    if args.check:
        ...
        for dest, text in outputs.items():
            if not dest.is_file() or dest.read_text(encoding="utf-8") != text:
                ...
                failed = True
        # covers
        wanted = {e["cover"] for e in entries}
    ...
    copied, removed, total = sync_covers(entries)
    for dest, text in outputs.items():
