...
def track_relative_id(album: str, index: int, title: str) -> str:
    """Track id = the path relative to the active music root.

    Mirrors the real library, which prefixes every catalog id with ``local_``
    no matter which root is active (library/core.py: f"local_{rel_path}"). The
    Folders view groups on this relative path, so each album becomes its own
    folder with its tracks inside instead of loose files under the root.
    """
    return f"local_{album}/{index:02d} - {title}.flac"
...
def favorite_flags(entries: list[dict]) -> dict[str, bool]:
    ...
    for e in entries:
        if e["library"] == "local":
            ...
            for i, t in enumerate(e["tracks"], start=1):
                tid = track_relative_id(e["album"], i, t["title"])
...
(function () {
    ...
    TRACK_SEEDS.forEach((seed) => {
        ...
        const fileName = String(existing.count).padStart(2, '0') + ' - ' + title + '.flac';
        // The id is the path relative to the music root, exactly like the real
        // library: the Folders view groups on it, so each album is its own
        // folder and its tracks live inside that folder.
        const id = 'local_' + album + '/' + fileName;
        ...
        tracks.push({
            ...
            disc_number: 1,
            source: 'local',
            url: `file://demo/${albumKey}/${fileName}`,
            duration,
            path: `demo/${album}/${fileName}`,
...
(function () {
    ...
    SEEDS.forEach((seed) => {
        ...
        trackSeeds.forEach(([title, duration], index) => {
            const fileName = String(index + 1).padStart(2, '0') + ' - ' + title + '.flac';
            // Id = path relative to this share's root, like the real library
            // (library/core.py: f"local_{rel_path}"), so the Folders view groups
            // one folder per album here too.
            const id = 'local_' + album + '/' + fileName;
            tracks.push({
                id,
                title,
                ...
                disc_number: 1,
                source: 'local',
                url: 'file://%s/' + albumKey + '/' + fileName,
                duration,
                path: '%s/' + album + '/' + fileName,
                sample_rate_hz: sampleRate,
                cover_available: true,
                cover_url: cover,
                cover_info_url: '/api/tracks/cover-info/' + id,
...
def render_library_js(entries: list[dict]) -> str:
    ...
    track_ids = [track_relative_id(e["album"], i + 1, t["title"])
                 for e in local for i, t in enumerate(e["tracks"])]
    n = len(track_ids)
    local_playlists = [
...
def smb_playlists(prefix: str, albums: list[dict], sampler_name: str) -> list[dict]:
    ...
    for e in albums:
        for i, t in enumerate(e["tracks"], start=1):
            by_id[track_relative_id(e["album"], i, t["title"])] = e
    ordered = list(by_id)
    ...
    seeds = [
        ...
        (f"playlist_{prefix}sampler", sampler_name,
         [track_relative_id(e["album"], 1, e["tracks"][0]["title"]) for e in albums]),
    ]
