#!/usr/bin/env python3
"""Cancel after winner apply must restore hardware, never report cancelled with winner active.

Reproduces the late-cancel window in the 2.1 runner: cancel arrives during
the gain_after sweep, after the winner delay was already applied. The runner
must observe the cancel before committing completion, restore the original
config, and leave UI (cancelled, no result) consistent with hardware
(original alignment).
"""
import copy
import sys
...
class AutoSubCancelAfterApplyTests(unittest.IsolatedAsyncioTestCase):
    async def test_late_cancel_restores_original_and_stays_cancelled(self):
        job_id = "auto-sub-cancel-after-apply"
        original = {
        ...
        async def noop_finish(_job, _job_id):
            return None
        ...
        try:
            with ExitStack() as stack:
                ...
                stack.enter_context(patch.object(runner, "_auto_sub_local_dip_recheck_decision",
                    return_value={"outcome": "final_kept", "incumbent_passed": True,
                ...
                stack.enter_context(patch.object(runner, "_finish_auto_sub_worker", side_effect=noop_finish))
            ...
            # Production finally block would finalize; exercise the real finalizer.
