...
class SamplerateOrchestrationContractTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_local_playback_handoff_shared_reconcile(self):
        ...
        self.assertLess(ledger.events.index("local.ensure-force:48000"), ledger.events.index("local.preset-sync"))
        self.assertLess(ledger.events.index("local.helper-sync"), ledger.events.index("commit-readback"))
    ...
    async def test_radio_post_load_handoff_switches_once(self):
        ...
        self.assertLess(ledger.events.index("radio.helper-sync"), ledger.events.index("commit-readback"))
        self.assertFalse(runtime.muted)
