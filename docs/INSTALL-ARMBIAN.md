...
- After saving, the applying page confirms the setup. The temporary setup network disappears during Wi-Fi onboarding, so reconnect the computer to the normal network when it goes away. The completion page prominently shows the stable post-install address `http://<device-name>.local:8000` (derived from `/etc/machine-id` as `fxroute-<6 chars>`, e.g. `http://fxroute-1af688.local:8000`) with an Open FXRoute link and a Copy address button. When the current IP address is known, the page additionally lists the reachable `http://<address>:8000` links; otherwise it names that address pattern for the router lookup. FXRoute itself appears at the `.local` address once first-boot installation finishes.
...
- After a fully completed installation, the console shows `FXRoute ready: http://<ip>:8000 and http://<device-name>.local:8000` with the current IPv4 address and the final `.local` address.- The installer creates the persistent systemd user session, PipeWire graph, native DSP engine, and `fxroute.service` as the target user.
