...
- After saving, the applying page confirms the setup. The temporary setup network disappears during Wi-Fi onboarding, so reconnect the computer to the normal network when it goes away. When the device address is known, the completion page links directly to `http://<address>:8000`; otherwise it names that address pattern for the router lookup. FXRoute itself appears there once first-boot installation finishes.
