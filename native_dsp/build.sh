...
if pkg-config --exists libpipewire-0.3; then
    ${CC:-cc} -std=c11 -O2 -Wall -Wextra -Werror -pedantic $(pkg-config --cflags libpipewire-0.3) dsp.c pipewire_engine.c $(pkg-config --libs libpipewire-0.3) -lm -pthread -o build/fxroute-dsp
else
